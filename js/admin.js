  ...
  for (const o of orders) {
    ...
    ticket.innerHTML = `
      ...
      <div style="margin-top:12px;display:flex;flex-wrap:wrap;gap:8px">
        ${statusButtons(o)}
        <button class="secondary save-btn">Save files to PC</button>
      </div>
    `;
...
async function saveOrderToPC(o) {
  if (!o.files || !o.files.length) {
    alert("No files to save yet.");
    return;
  }
  if (!window.showDirectoryPicker) {
    alert("Your browser doesn't support saving folders directly (use Chrome or Edge). You can still print files individually.");
    return;
  }
  try {
    const rootHandle = await window.showDirectoryPicker();
    const folderHandle = await rootHandle.getDirectoryHandle(sanitizeFolderName(o.folder_name), { create: true });
    for (const f of o.files) {
      const url = await SB.fetchFileUrl(f.path);
      const blob = await (await fetch(url)).blob();
      const fileHandle = await folderHandle.getFileHandle(f.name, { create: true });
      const writable = await fileHandle.createWritable();
      await writable.write(blob);
      await writable.close();
      URL.revokeObjectURL(url);
    }
    alert(`Saved ${o.files.length} file(s) into "${o.folder_name}".`);
  } catch (e) {
    if (e.name !== "AbortError") alert("Couldn't save files: " + e.message);
  }
}
